...
exports.FooBarService = void 0;
const core_1 = require("@aries-framework/core");
const FooBarEvents_1 = require("../FooBarEvents");
const FooBarRole_1 = require("../FooBarRole");
const messages_1 = require("../messages");
const models_1 = require("../models");
const repository_1 = require("../repository");
let FooBarService = class FooBarService {
    constructor(questionBarRepository, eventEmitter, logger) {
     ...
     */
    async createFoo(agentContext, connectionId, config) {
        const questionMessage = new messages_1.FooMessage({
        ...
        const questionBarRecord = await this.createRecord({
            ...
            role: FooBarRole_1.FooBarRole.Fooer,
            signatureRequired: false,
            state: models_1.FooBarState.FooSent,
            validResponses: questionMessage.validResponses,
        });
        await this.questionBarRepository.save(agentContext, questionBarRecord);
        this.eventEmitter.emit(agentContext, {
            type: FooBarEvents_1.FooBarEventTypes.FooBarStateChanged,
     ...
     */
    async processReceiveFoo(messageContext) {
        ...
        if (questionRecord) {
            throw new core_1.AriesFrameworkError(`Foo answer record with thread Id ${questionMessage.id} already exists.`);
        }
        const questionBarRecord = await this.createRecord({
            questionText: questionMessage.questionText,
            questionDetail: questionMessage.questionDetail,
            ...
            threadId: questionMessage.threadId,
            role: FooBarRole_1.FooBarRole.Responder,
            signatureRequired: false,
            state: models_1.FooBarState.FooReceived,
            validResponses: questionMessage.validResponses,
        });
     ...
     */
    async createBar(agentContext, questionBarRecord, response) {
        const answerMessage = new messages_1.BarMessage({ response: response, threadId: questionBarRecord.threadId });
        questionBarRecord.assertState(models_1.FooBarState.FooReceived);
        questionBarRecord.response = response;
        if (questionBarRecord.validResponses.some((e) => e.text === response)) {
            await this.updateState(agentContext, questionBarRecord, models_1.FooBarState.BarSent);
        }
        else {
     ...
     * @returns FooBar record
     */
    async receiveBar(messageContext) {
        const { message: answerMessage } = messageContext;
        this.logger.debug(`Receiving answer message with id ${answerMessage.id}`);
        const connection = messageContext.assertReadyConnection();
        const questionBarRecord = await this.findByThreadAndConnectionId(messageContext.agentContext, connection.id, answerMessage.threadId);
        if (!questionBarRecord) {
            throw new core_1.AriesFrameworkError(`Foo Bar record with thread Id ${answerMessage.threadId} not found.`);
        }
        questionBarRecord.assertState(models_1.FooBarState.FooSent);
        questionBarRecord.assertRole(FooBarRole_1.FooBarRole.Fooer);
        questionBarRecord.response = answerMessage.response;
        await this.updateState(messageContext.agentContext, questionBarRecord, models_1.FooBarState.BarReceived);
        return questionBarRecord;
    }
    /**
     ...
     *
     */
    async updateState(agentContext, questionBarRecord, newState) {
    ...
    async createRecord(options) {
        const questionMessageRecord = new repository_1.FooBarRecord({
     ...
     *
     */
    getById(agentContext, questionBarId) {
     ...
     *
     */
    findById(agentContext, questionBarId) {
...
FooBarService = __decorate([
    (0, core_1.injectable)(),
    __param(2, (0, core_1.inject)(core_1.InjectionSymbols.Logger)),
    __metadata("design:paramtypes", [repository_1.FooBarRepository,
        core_1.EventEmitter, Object])
], FooBarService);
exports.FooBarService = FooBarService;
//# sourceMappingURL=FooBarService.js.map